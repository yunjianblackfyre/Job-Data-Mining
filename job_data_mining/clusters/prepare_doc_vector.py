    ...
    if line:
        ...
        for word, flag in words_info:
            if flag in mark_tag_set and len(word) > 1: # 一个字的不要
                ...
                elif (word not in STOP_WORDS) and (word not in ZHILIAN_STOP_WORDS):
                    keyword_list.append(word)

class CPrepareDocVector(BatchProc):

    def __init__( self ):
        super(CPrepareDocVector, self).__init__("prepare_doc_vector",3)
        self.word_detail = {}
        self.doc_hidden_info_dict = {}
    ...
    def __compute_docvector(self, item):
        ...
        # 结果写入文档隐藏信息
    def __load_hidden_info(self,item):
        doc_url =           item["Fh5_url"]
        doc_title =         item["Ftitle"]
        doc_vector_str =    self.firelinker["Farticle_vector"]
        doc_tags =          self.firelinker["Ftag_detail"]
        doc_ready =         self.firelinker["Fdoc_ready"]
        self.doc_hidden_info_dict[doc_url] = (
            doc_vector_str,
            doc_title,
            doc_tags,
            doc_ready
    ...
    def wind_up(self):
        with open(doc_hidden_info_path,'w') as f:
            f.write(json.dumps(self.doc_hidden_info_dict))
    ...
    def run(self):
        ...
        while(True):
            where = "Fauto_id between %s and %s"%(start, end)       #t_job_documents已经完成图片下载
